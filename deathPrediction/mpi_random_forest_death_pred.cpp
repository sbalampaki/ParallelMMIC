...
//   Each MPI process builds a disjoint subset of the numTrees trees using
//   globally-consistent seeds (tree t always uses the same seed regardless of
//   which rank builds it).  For evaluation each process predicts votes for the
//   full test set using only its local trees, then MPI_Allreduce sums the vote
//   counts so every process can compute the global majority decision.
// ─────────────────────────────────────────────────────────────────────────────
...
public:
    RandomForestClassifierMPI(int nTrees, int maxD, int minSamples, int r, int s)
    ...
    void train(const vector<Patient>& data, unsigned int seed = 42) {
        ...
        // Distribute trees evenly across MPI processes
        ...
        for (int t = treeStart; t < treeEnd; t++) {
            // Use globally consistent seed so tree t is always the same tree
            ...
            localTrees.push_back(tree);
        }
    }

    // Each process accumulates vote counts for the test set from its own trees,
    // then MPI_Allreduce sums votes globally.
    double calculateAccuracy(const vector<Patient>& testData) {
        ...
        for (int i = 0; i < n; i++) {
            for (const auto* t : localTrees) {
                localVotes[i] += t->predict(testData[i]);
            }
        }

        // Sum votes across all processes
    ...
    double calculateDeathRate(const vector<Patient>& testData) {
        ...
        double total = 0.0;
        for (int i = 0; i < n; i++) {
            total += (double)globalVotes[i] / numTrees;
        }
        return total / n;
    }
};
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        cout << "MPI Implementation - Random Forest" << endl;
        cout << "===================================" << endl;
        cout << "Number of processes: " << mpiSize << endl;
    ...
    if (rank == 0) {
        ...
        cout << "Model: Random Forest (50 trees distributed across " << mpiSize << " processes)" << endl;
    }

    auto startTrain = chrono::high_resolution_clock::now();
    RandomForestClassifierMPI model(50, 10, 5, rank, mpiSize);
    ...
    if (rank == 0) {
        ...
        ofstream timingFile("timing_rf_mpi.txt");
