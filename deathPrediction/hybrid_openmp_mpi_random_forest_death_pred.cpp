...
// ─── Hybrid OpenMP + MPI Random Forest ───────────────────────────────────────
//
// Parallelism strategy:
//   MPI:   Each rank is assigned a disjoint slice of the numTrees trees.
//   OpenMP: Within each MPI rank the assigned trees are built in parallel using
//           #pragma omp parallel for (trees are still independent).
//   Evaluation: each rank accumulates local vote counts; MPI_Allreduce sums
//               votes globally so every rank can compute accuracy / death rate.
// ─────────────────────────────────────────────────────────────────────────────
class RandomForestHybridOMPMPI {
private:
...
public:
    RandomForestHybridOMPMPI(int nTrees, int maxD, int minSamples, int r, int s)
        : numTrees(nTrees), maxDepth(maxD), minSamplesSplit(minSamples), rank(r), mpiSize(s) {
        int treesPerProc = numTrees / mpiSize;
        int treeStart    = rank * treesPerProc;
        int treeEnd      = (rank == mpiSize - 1) ? numTrees : (rank + 1) * treesPerProc;
        localTrees.resize(treeEnd - treeStart, nullptr);
    }

    ~RandomForestHybridOMPMPI() {
    ...
    void train(const vector<Patient>& data, unsigned int seed = 42) {
        ...
        int treeStart    = rank * treesPerProc;
        int treeEnd      = (rank == mpiSize - 1) ? numTrees : (rank + 1) * treesPerProc;
        int localCount   = treeEnd - treeStart;

        // ── OpenMP: build assigned trees in parallel within this MPI rank ──
        #pragma omp parallel for schedule(dynamic)
        for (int li = 0; li < localCount; li++) {
            int t = treeStart + li;
            mt19937 treeRng(seed + (unsigned int)t * 1000003u);
            uniform_int_distribution<int> dist(0, n - 1);

            vector<Patient> bootstrap;
            bootstrap.reserve(n);
            for (int i = 0; i < n; i++) {
                bootstrap.push_back(data[dist(treeRng)]);
            }

            DecisionTree* tree = new DecisionTree(maxDepth, minSamplesSplit);
            tree->train(bootstrap, &treeRng);
            localTrees[li] = tree;  // pre-allocated, no race condition
        }
    }
    ...
    double calculateAccuracy(const vector<Patient>& testData) {
        ...
        // ── OpenMP: accumulate local votes in parallel ─────────────────────
        vector<int> localVotes(n, 0);
        #pragma omp parallel for schedule(static)
        for (int i = 0; i < n; i++) {
            int v = 0;
            for (const auto* t : localTrees) v += t->predict(testData[i]);
            localVotes[i] = v;
        }

        // ── MPI: sum vote counts across all ranks ──────────────────────────
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        if (rank == 0) {
            cerr << "Usage: mpirun -np <processes> " << argv[0] << " <data_file>" << endl;
        }
        MPI_Finalize();
        return 1;
    }
    ...
    if (rank == 0) {
        cout << "Hybrid OpenMP+MPI Implementation - Random Forest" << endl;
        cout << "=================================================" << endl;
    ...
    if (rank == 0) {
        ...
        cout << "Model: Random Forest (50 trees, MPI distributed + OpenMP parallel tree building)" << endl;
    }

    auto startTrain = chrono::high_resolution_clock::now();
    RandomForestHybridOMPMPI model(50, 10, 5, rank, mpiSize);
    ...
    if (rank == 0) {
        ...
        ofstream timingFile("timing_hybrid_openmp_mpi_rf.txt");
        ...
        timingFile << "threads,"   << omp_get_max_threads() << endl;
        timingFile.close();
    }
