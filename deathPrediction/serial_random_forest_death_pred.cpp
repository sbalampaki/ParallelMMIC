...
// Decision Tree used as a base learner in the Random Forest
class DecisionTree {
private:
    TreeNode* root;
    int maxDepth;
    int minSamplesSplit;
    mt19937* rng;  // pointer to caller-supplied RNG for feature subsampling
    ...
    // Choose 2 out of 3 features randomly (sqrt(3) ≈ 2)
...
public:
    RandomForestClassifier(int nTrees = 50, int maxD = 10, int minSamples = 5)
    ...
    void train(const vector<Patient>& data, unsigned int seed = 42) {
        ...
        for (int t = 0; t < numTrees; t++) {
            // Per-tree RNG for both bootstrap sampling and feature subsampling
            mt19937 treeRng(seed + (unsigned int)t * 1000003u);
            uniform_int_distribution<int> dist(0, n - 1);

            // Bootstrap sample (sample n records with replacement)
    ...
    // Predict by majority vote (returns 1 if >= half the trees vote for death)
    int predict(const Patient& p) const {
        int votes = 0;
    ...
    // Predicted probability = fraction of trees that vote for death
...
int main(int argc, char* argv[]) {
    ...
    cout << "Serial Implementation - Random Forest" << endl;
    cout << "======================================" << endl;
    ...
    cout << "Model: Random Forest (50 trees, max_depth=10, bootstrap, 2/3 features per split)" << endl;

    auto startTrain = chrono::high_resolution_clock::now();
    RandomForestClassifier model(50, 10, 5);
    ...
    ofstream timingFile("timing_rf_serial.txt");
