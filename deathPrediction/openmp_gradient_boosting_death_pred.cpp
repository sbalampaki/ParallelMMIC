...
// Shallow regression tree used in each boosting round
    ...
    void findBestSplit(const vector<Patient>& data,
        ...
        for (const auto& kv : ethSet) {
            ...
            for (size_t i = 0; i < data.size(); i++) {
                if (data[i].ethnicity == kv.first) lRes.push_back(residuals[i]);
        ...
        for (const auto& kv : genSet) {
            vector<double> lRes, rRes;
            for (size_t i = 0; i < data.size(); i++) {
                if (data[i].gender == kv.first) lRes.push_back(residuals[i]);
                else                             rRes.push_back(residuals[i]);
            }
            double gain = varianceReduction(residuals, lRes, rRes);
            if (gain > bestGain) {
        ...
        for (const auto& kv : icdSet) {
            vector<double> lRes, rRes;
            for (size_t i = 0; i < data.size(); i++) {
                if (data[i].icd9Code1 <= kv.first) lRes.push_back(residuals[i]);
                else                                rRes.push_back(residuals[i]);
            }
            double gain = varianceReduction(residuals, lRes, rRes);
            if (gain > bestGain) {
...
// ─── OpenMP Gradient Boosting Classifier ─────────────────────────────────────
//
// Parallelism strategy:
//   1. Pseudo-residual computation is parallelised with OpenMP (embarrassingly
//      parallel across samples).
//   2. After all trees are trained (sequentially, since each round depends on
//      the previous), the evaluation / prediction loop over the test set is
//      also parallelised.
// ─────────────────────────────────────────────────────────────────────────────
class GradientBoostingClassifierOMP {
private:
...
public:
    GradientBoostingClassifierOMP(int nTrees = 100, double lr = 0.1, int depth = 3)
        : F0(0.0), learningRate(lr), numTrees(nTrees), maxDepth(depth) {}

    ~GradientBoostingClassifierOMP() {
    ...
    void train(const vector<Patient>& data) {
        ...
        for (int t = 0; t < numTrees; t++) {
            // ── Step 1: compute pseudo-residuals in parallel ──────────────
            ...
            // ── Step 2: fit regression tree (sequential – data-dependent) ─
            RegressionTree* tree = new RegressionTree(maxDepth, 5);
            tree->train(data, residuals);
            trees.push_back(tree);

            // ── Step 3: update predictions in parallel ────────────────────
            #pragma omp parallel for schedule(static)
            for (int i = 0; i < n; i++) {
...
vector<Patient> loadData(const string& filename) {
    ...
    while (getline(file, line)) {
        ...
        try {
            ...
            if (!getline(ss, temp, ',')) continue; // HADM_ID

            if (!getline(ss, temp, ',')) continue;
            removeQuotes(temp);
            p.expireFlag = safeStoi(temp);

            if (!getline(ss, temp, ',')) continue; // ADMITTIME
            ...
            if (!getline(ss, temp, ',')) continue; // DOB
            if (!getline(ss, temp, ',')) continue; // AGE
...
int main(int argc, char* argv[]) {
    ...
    cout << "OpenMP Implementation - Gradient Boosting" << endl;
    cout << "==========================================" << endl;
    cout << "Number of OpenMP threads: " << numThreads << endl;
    ...
    GradientBoostingClassifierOMP model(100, 0.1, 3);
    ...
    // Save timing to file
    ofstream timingFile("timing_gb_openmp.txt");
