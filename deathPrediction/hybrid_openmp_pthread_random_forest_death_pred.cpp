...
struct TrainThreadData {
    ...
    vector<DecisionTree*>* trees;  // pre-allocated
    int treeStart;                 // global tree indices for this thread
    int treeEnd;
    int maxDepth;
    int minSamplesSplit;
    unsigned int seed;
...
// ─── Hybrid OpenMP + Pthread Random Forest ────────────────────────────────────
//
// Parallelism strategy:
//   OpenMP divides the numTrees trees into coarse-grained blocks, one per
//   OpenMP thread.  Each OpenMP thread then launches numPThreads Pthreads that
//   further sub-divide its block for fine-grained tree building.
//   Evaluation parallelises the test-set vote loop with OpenMP reductions.
// ─────────────────────────────────────────────────────────────────────────────
class RandomForestHybridOMPPThread {
private:
...
public:
    RandomForestHybridOMPPThread(int nTrees = 50, int maxD = 10, int minSamples = 5, int nPthreads = 2)
        : numTrees(nTrees), maxDepth(maxD), minSamplesSplit(minSamples),
          numPThreads(nPthreads > 0 ? nPthreads : 1) {
        trees.resize(numTrees, nullptr);
    }

    ~RandomForestHybridOMPPThread() {
        for (auto* t : trees) delete t;
    }

    void train(const vector<Patient>& data, unsigned int seed = 42) {
        int ompThreads  = omp_get_max_threads();
        int ompChunk    = (numTrees + ompThreads - 1) / ompThreads;

        // ── OpenMP: coarse-grained split of numTrees ───────────────────────
        #pragma omp parallel
        {
            int tid        = omp_get_thread_num();
            int ompStart   = tid * ompChunk;
            int ompEnd     = min(ompStart + ompChunk, numTrees);
            if (ompStart >= numTrees) { /* nothing for this thread */ }
            else {
                int ompCount   = ompEnd - ompStart;
                int ptChunk    = (ompCount + numPThreads - 1) / numPThreads;

                // ── Pthreads: fine-grained split within the OpenMP block ───
                ...
                for (int pt = 0; pt < numPThreads; pt++) {
                    ...
                    tdArray[pt].trees           = &trees;
                    tdArray[pt].treeStart       = ompStart + ptLocalStart;
                    tdArray[pt].treeEnd         = ompStart + ptLocalEnd;
    ...
    int predict(const Patient& p) const {
        int votes = 0;
        for (const auto* t : trees) if (t) votes += t->predict(p);
        return (votes * 2 >= numTrees) ? 1 : 0;
    }

    double predictProba(const Patient& p) const {
        int votes = 0;
        for (const auto* t : trees) if (t) votes += t->predict(p);
        return (double)votes / numTrees;
    }
    ...
    double calculateAccuracy(const vector<Patient>& data) const {
        ...
        for (int i = 0; i < n; i++) {
            if (predict(data[i]) == data[i].expireFlag) correct++;
        }
...
int main(int argc, char* argv[]) {
    ...
    cout << "Hybrid OpenMP+Pthread Implementation - Random Forest" << endl;
    cout << "=====================================================" << endl;
    ...
    cout << "Model: Random Forest (50 trees, OpenMP coarse + Pthread fine parallel tree building)" << endl;

    auto startTrain = chrono::high_resolution_clock::now();
    RandomForestHybridOMPPThread model(50, 10, 5, numPThreads);
    ...
    ofstream timingFile("timing_hybrid_openmp_pthread_rf.txt");
