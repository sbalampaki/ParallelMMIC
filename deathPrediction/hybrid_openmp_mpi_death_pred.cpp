...
class LogisticRegressionHybridOMPMPI {
private:
...
public:
    LogisticRegressionHybridOMPMPI(int r, int s) : bias(0.0), rank(r), size(s) {}
    ...
    void train(const vector<Patient>& data, int epochs = 100, double lr = 0.01) {
        ...
        // Gradient descent with hybrid MPI + OpenMP
        for (int epoch = 0; epoch < epochs; epoch++) {
            ...
            // OpenMP: Parallelize gradient computation within each MPI process
            #pragma omp parallel
            {
                double thread_bias = 0.0;
                map<string, double> thread_eth;
                map<string, double> thread_gen;
                map<int, double> thread_icd;
                ...
                for (int i = start; i < end; i++) {
                    ...
                    thread_bias += error;
                    thread_eth[p.ethnicity] += error;
                    thread_gen[p.gender] += error;
                    thread_icd[p.icd9Code1] += error;
                }
                
                // OpenMP reduction: Combine thread-local gradients
                #pragma omp critical
                {
                    localBiasGrad += thread_bias;
                    for (const auto& kv : thread_eth) localEthGrads[kv.first] += kv.second;
                    for (const auto& kv : thread_gen) localGenGrads[kv.first] += kv.second;
                    for (const auto& kv : thread_icd) localIcdGrads[kv.first] += kv.second;
                }
            }
            ...
            // For simplicity, update weights based on local gradients
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        cout << "Hybrid OpenMP+MPI Implementation - Logistic Regression" << endl;
        cout << "=======================================================" << endl;
    ...
    LogisticRegressionHybridOMPMPI model(rank, size);
    ...
    if (rank == 0) {
        ...
        // Save timing to file
        ofstream timingFile("timing_hybrid_openmp_mpi.txt");
