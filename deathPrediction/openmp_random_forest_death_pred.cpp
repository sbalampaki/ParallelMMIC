...
//   1. Each tree is independent → build all numTrees trees in parallel using
//      #pragma omp parallel for.  Each OpenMP thread gets its own RNG seeded
//      from (globalSeed + treeIndex) so results are deterministic regardless
//      of thread count.
//   2. Prediction/accuracy evaluation is also parallelised with OpenMP
//      reductions over the test set.
// ─────────────────────────────────────────────────────────────────────────────
class RandomForestClassifierOMP {
private:
...
public:
    RandomForestClassifierOMP(int nTrees = 50, int maxD = 10, int minSamples = 5)
        : numTrees(nTrees), maxDepth(maxD), minSamplesSplit(minSamples) {
        trees.resize(numTrees, nullptr);
    }

    ~RandomForestClassifierOMP() {
    ...
    void train(const vector<Patient>& data, unsigned int seed = 42) {
        ...
        for (int t = 0; t < numTrees; t++) {
            // Thread-local RNG seeded from (seed + t) for reproducibility
            ...
            trees[t] = tree;  // pre-allocated vector – no race condition
        }
    }

    int predict(const Patient& p) const {
        int votes = 0;
        for (const auto* t : trees) votes += t->predict(p);
    ...
    double calculateAccuracy(const vector<Patient>& data) const {
        int n = (int)data.size();
        int correct = 0;
        // ── Parallelise evaluation over test samples ───────────────────────
...
int main(int argc, char* argv[]) {
    ...
    cout << "OpenMP Implementation - Random Forest" << endl;
    cout << "======================================" << endl;
    ...
    cout << "Model: Random Forest (50 trees, max_depth=10, parallel tree building)" << endl;

    auto startTrain = chrono::high_resolution_clock::now();
    RandomForestClassifierOMP model(50, 10, 5);
    ...
    ofstream timingFile("timing_rf_openmp.txt");
